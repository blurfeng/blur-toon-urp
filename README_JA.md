...
- [✨ 機能](#-機能)
  ...
  - [Per Object Shadow オブジェクト単位のシャドウ](#per-object-shadow-オブジェクト単位のシャドウ)
    - [合成方式](#合成方式)
    - [Debug Mode デバッグモード](#debug-mode-デバッグモード)
...
> 以上はいずれも URP カスケードシャドウマップという 1 枚の上での補正であり、キャラクターがそのマップ内で確保できるテクセル数に制約される。それでも階段やエッジの飛びが許容できない場合は [Per Object Shadow オブジェクト単位のシャドウ](#per-object-shadow-オブジェクト単位のシャドウ)を参照。キャラクターへ高解像度のシャドウタイルを個別に割り当て、テクセル密度を根本から引き上げる。  
...
### Per Object Shadow オブジェクト単位のシャドウ
`Renderer Feature + シーンコンポーネント。マテリアルエディター UI には含まれない`

キャラクターごとに、そのバウンディングボックスへ密着した高解像度のシャドウタイルを個別に割り当て、キャラクタースケールにおける URP カスケードシャドウマップのテクセル密度不足を解決する。シャドウのエッジには**一切ソフト化を行わない**ため、NPR のハードエッジはそのまま保たれる。

**何を解決するのか**

本プロジェクトの設定を例にすると（シャドウ距離 150、4 カスケード、アトラス 4096 → 各カスケード 2048）、カスケード 0 のバウンディングスフィアの直径は約 13~14 m であり、それを 2048 に割り当てると約 `6.4 mm/テクセル` にしかならず、身長 1.8 m のキャラクターは約 280 テクセルしか占められない。ここから 2 つの問題が生じる：

| 症状 | 原因 |
| :-- | :-- |
| NPR のハードエッジにテクセルの階段が露出する | 1080p でキャラクターが高さ 900 px を占めるとき、シャドウ 1 テクセル ≈ 画面 3 ピクセル |
| ライトやキャラクターの移動でエッジがフレームごとに飛ぶ | カスケードのテクセルグリッドが、ライト方向とカメラ位置に応じて再量子化される |

本機能はキャラクターにタイルを 1 枚占有させる：アトラス 2048 の場合、画面内 1 体 → タイル 2048（2.2 m のキャラクターに対して約 `1.1 mm/テクセル`）、4 体 → タイル 1024（約 `2.1 mm/テクセル`）となり、密度は 3~6 倍に向上する。バウンディング半径の量子化とテクセルスナップを併用することで、キャラクター移動時のエッジの這い回りもなくなる。

**導入手順**

1. URP Renderer アセット（本プロジェクトでは `Assets/Settings/URP-HighFidelity-Renderer.asset`）で `Add Renderer Feature` → `BlurToon Per Object Shadow` を追加する；
2. キャラクターのルートノードに `BlurToonURP/Per Object Shadow Caster` コンポーネントをアタッチする；
3. メインライトの `Shadow Type` が `No Shadows` でないこと、およびキャラクターマテリアルの `シャドウ投射` が有効なままであることを確認する（オフにするとそのキャラクターにセルフシャドウは出ない）。

> コンポーネントの Inspector には**登録状態**と現在のバウンディングスフィアがそのまま表示される。キャスターは静的リストを介して Feature へ登録されるため、登録に失敗すると（コンポーネントが無効、シーンインスタンスではなくプレハブアセットを編集中、など）機能全体がエラーひとつ出さずに黙って無効化される。そのため状態をパネル上に出している。  

**動作の仕組み**

- 実行タイミングは `AfterRenderingShadows`。URP のメインライトシャドウの後、不透明オブジェクトの前に位置し、2 枚のシャドウマップは互いに干渉しない。`Game` と `SceneView` カメラにのみ適用される。
- タイルは**マテリアル自身の `ShadowCaster` Pass** で描画するため、`Lit.shader` に Pass を追加する必要はない。マテリアルで `シャドウ投射` をオフにしたパーツは同様にタイルへ入らず、シーンシャドウと同じ意味になる。
- キャスター自身のレンダラーを `cmd.DrawRenderer` で 1 つずつ明示的に発行し、`context.DrawShadows` は使わない。後者は Unity のシャドウキャスターカリングに依存しており、実測では特定のライト角度でジオメトリがまるごと破棄され、セルフシャドウが突然消える現象が起きた。
- その代償として、**タイルにはキャスター自身しか含まれず、シーンの遮蔽物は入らない**。キャラクターが受けるシーンシャドウは引き続き URP カスケードマップが担当し、両者をどう合成するかは `Combine Mode` が決める（下記の[合成方式](#合成方式)を参照）。
- アトラスは**そのフレームで実際に選ばれたキャスター数**に応じて `1×1 / 2×2 / 4×4` へ動的に分割される：画面にキャラクターが 1 体だけならアトラス全体を占有し、アトラスのメモリを増やさずにテクセル密度を倍にできる。代償として、選出数が 1/4/16 の境界をまたぐとタイルサイズが変わり、そのフレームでシャドウ品質が一度だけ変化する。
- キーワード `_BLURTOON_PER_OBJECT_SHADOW` はフレームごとに切り替わる。キャスターがないフレームでは完全にオフになり、Shader は URP シャドウへフォールバックしてコストはゼロ。本 Feature を追加していないプロジェクトでは定数バッファすら宣言されない。

**Feature のパラメーター**（URP Renderer アセット上）

| グループ | パラメーター | 既定値 | 説明 |
| :-- | :-- | :-- | :-- |
| Atlas | `Atlas Size` | `2048` | シャドウアトラスの総解像度（512 / 1024 / 2048 / 4096） |
| Atlas | `Max Caster Count` | `4` | 同画面で処理するキャスターの最大数（1~16）。超過分は優先度順に切り捨てられ、URP カスケードシャドウへフォールバックする |
| Direction | `Direction Mode` | `ViewBlend` | `MainLight`＝メインライトに厳密に追従。物理的には正しいが、ライト回転時にテクセルグリッド全体が回転する。`ViewBlend`＝「キャラクター→カメラ」の視線方向を主とし、メインライト方向を比率でブレンド。ライトが回っても投影基底がほぼ変わらず、エッジが極めて安定する |
| Direction | `View Blend To Light` | `0.2` | `ViewBlend` でメインライト方向をブレンドする比率。0＝完全に視線追従（最も安定するがシャドウがほとんどライトに反応しない）、1＝`MainLight` と同等 |
| Direction | `View Blend Pitch Clamp Degrees` | `(90, 150)` | `ViewBlend` でシャドウ方向とワールド上方向のなす角（度）を制限し、真上からの見下ろし/真下からの見上げで不自然なセルフシャドウが出るのを防ぐ |
| Range | `Max Distance` | `50` | この距離（m）を超えたキャスターにはタイルを割り当てない |
| Range | `Fade Range` | `5` | `Max Distance` の何 m 手前から URP カスケードシャドウへフェードし始めるか。急な切り替わりを防ぐ |
| Range | `Caster Extrusion` | `2` | 平行投影の視錐台をライト方向へ余分に伸ばす距離（m）。ニアプレーンのマージンを広げる用途。長い武器やマントなどバウンディングスフィアからはみ出るパーツがある場合に大きくする |
| Range | `Combine Mode` | `SceneAndSelf` | URP メインライトシャドウとの合成方式。下記の[合成方式](#合成方式)を参照 |
| Range | `Self Reject Scale` | `1` | `SceneAndSelf` 専用：自己除外距離の倍率。下記の[合成方式](#合成方式)を参照 |
| Bias | `Depth Bias` / `Slope Bias` | `1.0` / `2.5` | ハードウェアラスタライズの深度バイアス（定数項 / 傾斜項）。頂点移動ではなくラスタライズバイアスを使うため、ライト方向によってキャスターのシルエットが変形せず、シャドウ境界が表面上を滑らない |
| Bias | `Normal Bias` | `0.5` | キャスター側のノーマルバイアス。単位は**タイルのテクセル**。法線方向へキャスターを内側に縮め、明暗の境界でセルフシャドウが砕けるのを抑える。タイル解像度が高いため、必要な値はカスケードシャドウよりはるかに小さい |
| Bias | `Receiver Normal Offset` | `1.5` | レシーバー側のノーマルオフセット。単位は**タイルのテクセル**。グレージング角では 1 テクセルが表面上の非常に長い区間を覆うため、キャスター側の bias をいくら調整しても追いつかず、面全体が一斉に自己遮蔽へ反転する（「ライトが面と平行になった瞬間に真っ黒になる」現象）。テクセルのワールドサイズに基づくレシーバー側の補正が、この構成で唯一有効な手段。大きすぎると表面に接した細かいシャドウが光漏れする |
| Appearance | `Strength` | `1` | シャドウの強度。1＝完全遮蔽、0＝シャドウを出さない |
| Appearance | `Stabilize Texel Snapping` | `オン` | 投影行列をテクセルグリッドへスナップし、キャラクター移動時にエッジが連続的に這うのではなく 1 テクセル単位で飛ぶようにする。キャスター側の `Radius Quantize` と併用する |
| Debug | `Debug Mode` | `Off` | タイルの内容をキャラクターへ直接描画する。下記の [Debug Mode デバッグモード](#debug-mode-デバッグモード)を参照。**リリース前に必ずオフにすること** |

**キャスターコンポーネントのパラメーター**（キャラクターのルートノード上）

| パラメーター | 既定値 | 説明 |
| :-- | :-- | :-- |
| `Renderers` | 空 | バウンディングボックス計算に含めるレンダラー。空のままなら有効化時に子階層のレンダラーを自動収集する。実行時に着せ替えなどでパーツを増減した場合は `RefreshRenderers()` を手動で呼ぶ |
| `Bounds Padding` | `0.1` | バウンディングボックスの外側への拡張（m）。動きが大きいときに手足がタイル境界からはみ出してシャドウが切れるのを防ぐ |
| `Radius Quantize` | `0.25` | バウンディング半径の量子化ステップ（m）。スケルタルアニメーションでバウンディングボックスはフレームごとに変化するため、平行投影のサイズもそれに追従するとテクセルサイズが毎フレーム変わり、テクセルスナップが完全に無意味になる。半径をこのステップへ切り上げると投影サイズが固定され、移動時のエッジの這い回りを消す鍵になる。0＝量子化なし（非推奨） |
| `Priority Bias` | `0` | 同画面のキャスターが上限を超えたとき、「カメラに近い + 画面中央寄り」の順で上位 N 体が選ばれる。この値はソート重みへ直接加算される（小さいほど優先）。主人公には負値を設定すれば常にタイルを確保できる |

> バウンディングスフィアの半径は AABB の対角線の半分を採用しており、どのライト方向でも平行投影がオブジェクト全体を覆うことを保証する。ライト方向ごとにフィッティングし直す必要はない（フィッティングし直すと投影サイズがライトに応じて変化し、それこそが避けたいジッターの原因になる）。オブジェクト選択時には Scene ビューにこのスフィアがワイヤーフレームで描画される。  

#### 合成方式
タイルにはキャスター自身しか含まれないため、`Combine Mode` はキャラクターがシーンシャドウを受けられるかどうかを直接決める：

| モード | 挙動 | 代償 |
| :-- | :-- | :-- |
| `Min` 暗い方を採用 | キャラクターは URP のシーンシャドウとタイルの高密度セルフシャドウの両方を受ける | URP カスケード側の低解像度なセルフシャドウが残るため、階段は完全には消えない |
| `Replace` ヒット時に置換 | タイルにヒットした時点で結果を完全に置き換える。セルフシャドウのエッジは最もきれい | **キャラクターはシーンから投げかけられるシャドウを受けられない。** シーンの遮蔽物がない演出カット（立ち絵、着せ替え画面）専用 |
| `SceneAndSelf`（既定、推奨） | シーンシャドウ + 高解像度セルフシャドウの両立 | シャドウマップ経路が必要。下記の注意を参照 |

`Min` と `Replace` のジレンマは同じ 1 点に由来する：**キャラクターが 2 枚のシャドウマップの両方に存在すること**。`SceneAndSelf` はカスケードマップをサンプリングする前に、サンプル点をメインライト方向へ本体から一定距離だけ押し出す（自己除外オフセット）。これによりキャラクター自身がカスケードマップへ書き込んだ深度は自分を遮蔽できなくなり、カスケードマップにはシーン遮蔽物の寄与だけが残る。そこへタイルのセルフシャドウと min を取ることで、2 つのシャドウがそれぞれの役割を果たし、互いを汚染しなくなる。

ライト方向への平行移動はライト空間の xy を変えないため、サンプリングされるテクセルは同じままであり、シャドウのパターンは**横方向にまったくずれない**。前へ動くのは深度比較の基準だけである。

自己除外の基準距離は Shader が**ピクセル単位**で算出する：そのピクセルからメインライト方向へどれだけ進めばキャラクターのバウンディングスフィアを抜け出るか（レイ―球の交差）。ライトを向いた面ではほぼ 0、背面ではスフィアの直径に近づく。`Self Reject Scale` はその倍率：

| 値 | 効果 |
| :-- | :-- |
| `1`（既定） | ちょうどバウンディングスフィアの境界まで除外する |
| 小さくする | URP 側の低解像度セルフシャドウが再び滲み出す |
| 大きくする | キャラクターに接近したシーン遮蔽物まで除外され、近くの物体がキャラクターへ影を落とせなくなる |
| `0` | 自己除外を完全に無効化。`Min` と同等 |

> ⚠ `SceneAndSelf` はワールド座標でカスケードマップを再サンプリングする必要があるため、**スクリーンスペースシャドウ**（URP Asset の `Screen Space Shadows`、キーワード `_MAIN_LIGHT_SHADOWS_SCREEN`）では実現できない。このモードは自動的に無効化され `Min` へフォールバックする。  
> 再サンプリング時はマテリアルの `低品質PCF` スイッチを尊重し、`ForwardLit` のメインライトシャドウのサンプリングと一致させる。  

#### Debug Mode デバッグモード
「このシャドウはどこから来ているのか」を追うときは、最終画面から推測するよりタイル自体を直接見る方がはるかに確実である。

| モード | 表示内容 | 読み方 |
| :-- | :-- | :-- |
| `Off` | オフ | — |
| `ShadowValue` | シャドウ値のグレースケール | 白＝遮蔽なし、黒＝完全遮蔽。細かいノイズや縞が出ればシャドウアクネ（bias 不足） |
| `TileCoverage` | タイルのカバレッジ | 緑＝ピクセルがタイル内、赤＝タイル外（URP シャドウへフォールバック）。キャラクターに赤が出ていればタイルが覆えていない |
| `TileUV` | タイルの UV | 赤緑のグラデーションがキャラクターを滑らかに覆っているべき。あるライト角度で突然全体がずれる/反転する場合は行列構築に不連続がある |
| `AtlasDepth` | アトラスの生の深度 | リバース Z では 1＝近、0＝遠、クリア値は遠(0)。キャラクターの位置が真っ黒 → ジオメトリがそもそもアトラスへ入っていない（カリングの問題）。深度のグラデーションが見える → タイルには中身があり、問題はサンプリング側にある |
| `ReceiverDepth` | レシーバー側の深度 | そのピクセルのタイル内 z。キャラクターを滑らかに覆い、開区間 (0,1) に収まっているべき。広い範囲が 0 や 1 に張り付く → 深度レンジがキャラクターを覆えていない |
| `TexelDensity` | テクセル密度のチェッカー | 1 マス＝シャドウ 1 テクセル。**「エッジのジャギーにまだ手が打てるか」を判断する客観的な根拠**：マスが画面 1 ピクセルより小さければ階段はピクセル単位であり、ポストプロセスのアンチエイリアスで解決できる。マスが画面 1 ピクセルより明らかに大きければ階段はピクセルより粗く、`Atlas Size` を上げる、同画面のキャスター数を減らす、バウンディングスフィアを小さくする以外に手はない |

**ソースコードの場所**

| ファイル | 役割 |
| :-- | :-- |
| `Core/Runtime/PerObjectShadow/BlurToonPerObjectShadowFeature.cs` | Renderer Feature のエントリーポイント |
| `Core/Runtime/PerObjectShadow/BlurToonPerObjectShadowPass.cs` | レンダリング Pass：選出、タイル分割、描画、定数のアップロード |
| `Core/Runtime/PerObjectShadow/BlurToonPerObjectShadowSettings.cs` | シリアライズ可能な設定と列挙型 |
| `Core/Runtime/PerObjectShadow/BlurToonPerObjectShadowUtils.cs` | 行列の構築、テクセルスナップ、優先度ソート |
| `Core/Runtime/PerObjectShadow/BlurToonPerObjectShadowCaster.cs` | キャスターコンポーネント |
| `Core/Editor/BlurToonPerObjectShadowCasterEditor.cs` | キャスターのインスペクター（登録状態の表示） |
| `Core/Shaders/PerObjectShadowFunction.hlsl` | Shader 側のサンプリング関数ライブラリ |
...
> `ShadowCaster` Pass は [Per Object Shadow オブジェクト単位のシャドウ](#per-object-shadow-オブジェクト単位のシャドウ)がオブジェクト単位のシャドウアトラスを描画する際にも使われる。そのため同機能は `Lit.shader` へ Pass を追加する必要がない。  
